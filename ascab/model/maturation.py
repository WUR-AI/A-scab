...
class PseudothecialDevelopment(nn.Module):
    ...
    def update_rate(self, df_weather_day):
        day = torch.tensor(df_weather_day.index.date[0].timetuple().tm_yday)
        avg_temperature = torch.tensor(df_weather_day['temperature_2m'].mean())
        total_rain = torch.tensor(df_weather_day['precipitation'].sum())
        hours_humid = torch.tensor(len(df_weather_day[df_weather_day['relative_humidity_2m'] > 85.0]))
        wetness_duration = torch.tensor(compute_leaf_wetness_duration(df_weather_day))
        self.rate = self.compute_rate_torch(self.value, day, avg_temperature, total_rain, hours_humid, wetness_duration)
        return self.rate

    def compute_rate_torch(self, current_value, day, avg_temperature, total_rain, humid_hours, wetness_duration):
        ...
        start_day = torch.tensor(datetime.datetime.strptime('February 1', '%B %d').timetuple().tm_yday)
...
class AscosporeMaturation(nn.Module):
    ...
    def update_rate(self, df_weather_day):
        precipitation = torch.tensor(df_weather_day['precipitation'].values)
        vapour_pressure_deficit = torch.tensor(df_weather_day['vapour_pressure_deficit'].values)
        temperature_2m = torch.tensor(df_weather_day['temperature_2m'].values)
        self.rate, self._delta_dhw = self.compute_rate_torch(self._dependencies.value, self._dhw, precipitation,
                                                             vapour_pressure_deficit, temperature_2m)
        return self.rate

    def compute_rate_torch(self, pseudothecia, current_dhw, precipitation, vapour_pressure_deficit, temperature_2m):
...
class LAI(nn.Module):
    ...
    def update_rate(self, df_weather_day):
        day = torch.tensor(df_weather_day.index.date[0].timetuple().tm_yday)
        avg_temperature = torch.tensor(df_weather_day['temperature_2m'].mean())
        self.rate = self.compute_rate_torch(self.value, day, avg_temperature)
        return self.rate
