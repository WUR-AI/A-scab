...
class LagrangianPPO(PPO):
    ...
    def train(self) -> None:
        ...
        self.logger.record("train/constraint_max_spray", np.mean(constraint_1))
        # self.logger.record("train/constraint_between_sprays", np.mean(constraint_2))
...
def max_action_constraint(actions,
                          episodic_step,
                          non_zero_action_counter,
                          last_non_zero_action_step,
                          max_non_zero_actions=8,
                          min_steps_between_actions=1,
                          coef=4.0,
):
    """
        ...
        - episodic_step: The current step number in the episode.
        - non_zero_action_counter: Maximum number of non-zero actions allowed.

        Returns:
        - constraint_values: The value representing the degree of constraint violation.
    ...
    constraint_1_violation = non_zero_action_counter >= max_non_zero_actions

    # Constraint 2: Ensure non-zero actions are spaced by at least `min_steps_between_actions`
    # steps_since_last_non_zero = episodic_step - last_non_zero_action_step
    # constraint_2_violation = np.logical_and(sampled_non_zero_actions,
    #                                         steps_since_last_non_zero < min_steps_between_actions)

    return constraint_1_violation
...
class RolloutBufferSteps(RolloutBuffer):
    ...
    def __init__(self, buffer_size, observation_space, action_space, device='cpu', gae_lambda=1, gamma=0.99, n_envs=1,
        ...
        self.calculate_costs = max_action_constraint
...
class CostActorCriticPolicy(ActorCriticPolicy):
    ...
    def _build(self, lr_schedule) -> None:
        ...
        self.cost_value_net = nn.Linear(self.mlp_extractor.latent_dim_cf, 1)
...
class CostMlpExtractor(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(net_arch, dict):
            ...
            cf_layers_dims = net_arch.get("cf", [])  # Layer sizes of the cost value network
        else:
