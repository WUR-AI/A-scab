...
from ascab.model.infection import InfectionRate, get_values_last_infections, get_discharge_date, \
    compute_leaf_development, will_infect, get_risk
...
def run_episode(dates, df_weather):
    ...
    for day in dates:
        ...
        result_data['Ascospores'].append(ascospore_value - pat_previous)
        ...
        result_data["Risk"].append(get_risk(infections, day.date()))
...
def simulate():
    ...
    weather_summary = summarize_weather(start_end, df_weather)
    results_df, infections = run_episode(start_end, df_weather)
